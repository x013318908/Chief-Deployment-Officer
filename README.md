...
- 保護ツール: `list_dir`, `read_file`, `write_file`, `create_dir`, `delete_file`, `delete_dir`, `rename_path`, `get_env_path`, `request_env_upload`
...
代わりに、ドキュメントルート外に本番用envファイルを1つだけ配置します。既定の保存先は `DOCUMENT_ROOT` の1つ上にある `.cdo-secrets/{cdo_app_root_hash}/production.env` です。候補一覧は返さず、CDOが単一の `envPath` を決めます。
...
ドキュメントルート配下にしか安全な保存先を確保できない場合、CDOはenv配置を拒否します。`.htaccess`, `.user.ini`, `php.ini`, FPM pool設定は動的生成しません。その場合は、ホスティング会社が提供する環境変数設定、Secrets設定、または管理画面の設定機能を使ってください。
